...
- **Mission Run SIM clean up (park all SITLs + optional teleport on complete):** product lock in **README** → **Mission Run SIM clean up (product lock)**. Wave concurrency for park / mission clear / geofence clear / battery: env ``GUARDIAN_MISSION_RUN_SIM_CLEANUP_MAX_CONCURRENT`` (see ``MissionRunSimCleanupConcurrency``).
