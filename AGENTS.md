...
- **Mission Run SIM clean up (park all SITLs + optional teleport on complete):** product lock in **README** → **Mission Run SIM clean up (product lock)**. Wave concurrency for SIM **kill** / mission clear / geofence clear / battery: env ``GUARDIAN_MISSION_RUN_SIM_CLEANUP_MAX_CONCURRENT`` (see ``MissionRunSimCleanupConcurrency``).
- **Mission task between-cycles (Return to Launch / Loiter / Park):** **README** → **MRE policy & between-cycles dispatch** and **MRE mission template persistence** (Fleet recipes / Layer 1 list); deferred **Swap In Replacement** action — ``NEXTVERSION.md`` (2026-05-13).
- **Mission geofences:** Authoring, planner merge, policy, map overlays, optional `geofencePolygonsJSON` on mission upload, standalone geofence atoms/recipes, geofence clear on RTL teardown and run-complete SIM cleanup, and **distinct MRE log templates** for geofence encode failures (`missionGeofencePolygonsEncodeFailed`) and geofence fleet ack failures (`missionRunGeofenceFleetAckFailed`, including composite mission-upload stacks that fail on the geofence child) — **README** mission-recipes bullet and **Mission Run SIM clean up**.
