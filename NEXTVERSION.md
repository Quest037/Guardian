...
- **Context:** Shipped v1 is documented in **README_FULL.md** → **Floating reserve pool (Mission Control run)**. Source anchors: `MissionRunEnvironment`, `MissionRunReservePool.swift`, `MissionControlSetupView` / `MissionRunDetailView`, `MissionRunPlannerSubsystem` (plan mutation vet), `MissionControlReservePoolMapMarkerID`.
