...
**Paladin start preflight (Mission Control setup → run):** `MissionRunStartPreflightSheet` runs arm probes in roster order, then **filled** per-task floating reserve pool slots (`MissionRunEnvironment/orderedStartRunPreflightProbeTargets()`). Empty pool berths are omitted. Like roster rows, the probe path requires a parsable **`FleetMissionVehicleToken`** storage key; legacy text-only bindings surface the same “pick from the fleet list” failure as roster slots without a token.
...
**MCS (setup):** `MissionRunDetailView` / `MissionControlSetupView` — per expanded task, a horizontal **Floating reserves** strip (`ScrollView`), `+` adds a slot, each cell reuses `MissionControlRosterSlotCard`; duplicate fleet token warnings across pool + roster; **swap** control when `swapRosterAssignmentWithRandomFloatingReserve` can succeed. Preflight order: `orderedStartRunPreflightProbeTargets()`.

**MRE (swap + plan):** ``swapRosterAssignmentWithRandomFloatingReserve`` + ``returnAssignmentToReservePool`` + log template `missioncontrol.mre.reserve.swap_engaged`. **Plan mutations:** ``MissionRunPlannerSubsystem`` post-vets ``MissionControlPlanMutation`` so a fleet token cannot be roster-bound while still held in any pool berth (`reservePoolContainsFleetVehicleStorageKey`), and task reassignment cannot move an assignment onto a task whose pool already holds that token.

**MCR (running):** Read-only **Floating reserves** chips live in the **task triage** sheet (`missionLiveTaskTriageInnerSheet`), not in the compact Tasks list row. **Live overview map:** fleet-bound pool slots with hub position render as vehicle markers (label `… · pool`); ids use ``MissionControlReservePoolMapMarkerID`` (`MissionControlReservePoolMapMarkerID.swift`); tap opens that task’s triage; **Show on map** from triage includes pool positions in fit bounds.

**Tests:** `MissionRunReservePoolTests`, `MissionRunPlannerReservePoolMutationTests`, `MissionControlReservePoolMapMarkerIDTests`.
...
**Live reserve swap-in** (replace active with reserve: arm, mission handoff, reposition, roster commit, disposition of replaced aircraft, class matching, escalation / manual / auto triggers; **pool + fixed `.reserve`**): **`MissionRosterReservesToDo.md`**.
