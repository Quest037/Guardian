...
**Reserve swap disposition of replaced active (v1)** — `MissionRunReserveSwapReplacedActiveDispositionPolicy.swift` (pool: ``MissionRunReserveSwapReplacedActivePoolDispositionPolicy``; fleet wind-down: ``MissionRunReserveSwapReplacedActiveFleetWindDownPolicy``; MC-R / map: ``MissionRunReserveSwapReplacedActiveTelemetryHandoffPolicy``). **Binding destination (swap-in commit):** ``MissionRunReserveSwapReplacedActiveReturnPathPolicy`` — floating pool swap-in writes the displaced roster binding onto **the same pool berth id** that supplied the reserve (in-place exchange; not ``returnAssignmentToReservePool``); fixed template reserve swap-in **exchanges** tokens between the vacancy row and the **source `.reserve` roster assignment** (pool unchanged). **Pool / written-off:** a successful floating swap still requires the vacating roster binding to be **eligible** to occupy the pool berth (same gates as ``returnAssignmentToReservePool`` for fleet rows); rejections fail the swap atomically — use ``markFleetVehicleWrittenOffForReservePool`` and operator alternate paths when a **vehicle** cannot sit in that pool row. **Fleet:** RTL / rally (nearest open mission point) / loiter / land-class behaviour uses the same ``MissionRunFleetDispatch`` catalogue + recipe atoms as complete/abort planning; orderly swap-out should prefer ``MissionRunPolicyResolution/resolvedCompletePreferenceChain`` and ``MissionRunExecutionSubsystem/buildCompletePolicyWindDownCommands(limitedToAssignmentIDs:)`` on the **row that still owns the replaced fleet token** (pool / reserve), not the vacancy row; abort-class stop uses ``MissionRunPlannerSubsystem/buildAbortPlan`` semantics. **Telemetry handoff:** running UI foregrounds the **vacancy** (new active); the replaced **vehicle** stays on the fleet map until wind-down completes or link is lost.
...
| ``policies`` | ``MissionRunAssignmentPolicies``: optional **non-empty** ``abortPreferenceChain`` / ``completePreferenceChain`` overrides (otherwise task → mission defaults apply via ``MissionRunPolicyResolution``). |
...
**Policy resolution:** ``MissionRunPolicyResolution/resolvedTaskId(for:mission:)`` returns explicit ``taskId``, else the sole **enabled** task’s id when exactly one enabled task exists, else **nil** (ambiguous). Abort/complete chains use **assignment → task → mission** (first non-empty wins, then normalised).
...
- **Layer 1 subsystems** — each subsystem owns a sibling directory under `Sources/GuardianHQ/Systems/Fleet/Subsystems/` containing a `@MainActor` registrations entry point invoked once by `FleetRecipesCatalogueBootstrap` plus a uniquely-named per-recipe-bodies directory. Directory names must be unique inside the bundle because SPM flattens `.copy(<dir>)` resources to the bundle root. Today's subsystems:
  ...
  - `Mission/FleetMissionRecipeRegistrations.swift` + `Mission/MissionBodies/*.json` — MRE / Mission Control recipes: `recipe.fleet.do.mission.upload.start` (upload → arm → start) and `recipe.fleet.do.return.home` (single-step `command.fleet.vehicle.do.return.home` / RTL). Descriptor metadata is Swift-authored; bodies load from JSON like other subsystems, but **if bundle JSON is missing** (some Xcode / host layouts), registration uses a **compiled-in `FleetRecipeBody`** for each of those names so the catalogue never stays empty. The entry-point enum exposes `bodiesSubdirectoryName` (`"MissionBodies"`).
