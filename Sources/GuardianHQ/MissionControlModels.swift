...
struct MissionRunPreflightSlotRow: Identifiable, Equatable {
    ...
    var detail: String
    /// Set when **`phase == .failed`** — operator hints from `ArmFailureAdvisor` (pattern-matched; extend in `ArmFailureAdvisor.swift`).
    var remediationAdvice: ArmFailureRemediationAdvice? = nil

    var id: UUID { assignmentID }
}

/// Result of a **single-vehicle** arm probe (Vehicles “Test arm” modal).
struct SingleVehicleArmProbeResult: Equatable {
    let passed: Bool
    ...
    let detail: String
    let remediationAdvice: ArmFailureRemediationAdvice?
}
