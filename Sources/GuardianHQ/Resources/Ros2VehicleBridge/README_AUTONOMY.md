...
- **Planner API:** Training uses Nav2 **`ComputePathToPose` action** (`/compute_path_to_pose` on Humble). Readiness probes and ``nav2_path_planner`` must not target the removed service type.
