...
- **Fleet Nav2:** `nav2_training.launch.py` (open-field map + `planner_server`) starts **at application launch** via a headless ROS bridge host (no PX4 vehicles required). Retries on timeout/launch error (Python + Swift). Calls `/planner_server/compute_path_to_pose`; Training debug rail shows **Nav2** vs **Python fallback**. Tune with `GUARDIAN_NAV2_READY_TIMEOUT_S` (default 120), `GUARDIAN_NAV2_MAX_START_ATTEMPTS`, `GUARDIAN_NAV2_RETRY_DELAY_S`.
