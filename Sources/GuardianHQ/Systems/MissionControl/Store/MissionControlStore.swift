...
final class MissionControlStore: ObservableObject {
    ...
    func startRun(
        ...
        if var session = paladinSessionsByRunID[id] {
            ...
            let staging = PaladinEngine.shared.executeMissionControlStagingPass(run: runs[idx], mission: mission)
    ...
    func handleAutopilotMissionCycleFinished(
        ...
        for runID in runningIDs {
            ...
            guard let mission = missions.first(where: { $0.id == run.missionId }),
                  let built = PaladinEngine.shared.buildMissionControlSingleDronePathMission(run: run, mission: mission),
    ...
    private func performScheduledMissionCycleRestart(
        ...
        let pass = PaladinEngine.shared.executeMissionControlPrimaryMissionPass(run: runs[idx], mission: mission, pathId: pathID)
    ...
    private func enqueueInitialPrimaryMissionPasses(
        ...
        let buildable: [BuildEntry] = orderedEnabled.compactMap { path in
            guard let built = PaladinEngine.shared.buildMissionControlDronePathMission(run: run, mission: mission, pathId: path.id)
            else { return nil }
    ...
    func compilePaladinSession(
        ...
        let plan = PaladinEngine.shared.compileMissionControlPlan(
            run: run,
            mission: mission,
        ...
        UserNotificationService.shared.notifyPaladinPlanCompiled(
            runID: run.id,
